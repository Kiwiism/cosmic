...
Combat v1 begins as a non-damaging preview. When combat policy is enabled,
`ATTACK` and `GRIND` select the nearest matching alive monster, move toward the
monster until inside conservative attack range, and then stop at `ATTACK_READY`.
No damage, EXP, or drops are generated by this adapter yet; it exists to prove
target selection, approach movement, policy gates, cooldowns, and Agent CMS
observability before wiring into Cosmic's validated attack pipeline.
