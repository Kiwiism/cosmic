...
function Agents(){
 const [agents,setAgents]=useState<any[]>([]),[selected,setSelected]=useState<any|null>(null),[q,setQ]=useState(""),[charQ,setCharQ]=useState(""),[chars,setChars]=useState<any[]>([]),[plan,setPlan]=useState<any|null>(null),[logs,setLogs]=useState<any[]>([]),[runtimeResult,setRuntimeResult]=useState<any|null>(null),[reason,setReason]=useState("Updated through Server CMS"),[error,setError]=useState("");
 ...
 const loadAgentDetails=()=>{if(!selected){setPlan(null);setLogs([]);return}api(`/api/agents/${selected.id}/spawn-plan`).then(setPlan).catch(()=>setPlan(null));api<any[]>(`/api/agents/${selected.id}/logs`).then(setLogs).catch(()=>setLogs([]))};
